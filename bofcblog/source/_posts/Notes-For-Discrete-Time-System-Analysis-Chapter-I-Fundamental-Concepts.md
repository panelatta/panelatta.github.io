...
#### **_Some typical examples of discrete-time signals_**

##### _Discrete-Time Unit-Step Function_
...
##### _Discrete-Time Unit-Ramp Function_
...
##### _Periodic Discrete-Time Signals_

For a discrete-time signal $x[n]$, if there exists a _positive_ integer $r$ which makes that $x[n+r]=x[n]$ **for all integers n**, Then $x[n]$ is called a **periodic discrete-time signal** and  the integer $r$ is **period**. **Fundamental period** is the smallest value for positive integer $r$.
...
> 
> Cause $\mathrm{cos}$ is periodic, there is $A\mathrm{cos}(\Omega n + \theta) = A\mathrm{cos}(\Omega n + 2 \pi q + \theta)$ for _all_ integers $q$ .
> 
> Obviously, $x[n]$ is periodic when there exists an integer $r$ which makes $\Omega r = 2 \pi q$ for _some_ integers $q$, in equivalent, the **discrete-time frequency** $\Omega = \dfrac{2 \pi q}{r}$ for some integers $q,r$.

##### _Discrete-Time Complex Exponential Signals_
...
##### _Discrete-Time Rectangular Pulse_
...
##### _Discrete-Time Signals defined Interval by Interval_
...
#### **_Definition of Discrete-Time Systems and Analysis_**
...
#### **_System with or without memory_**
